...
import { ShoppingBag, Search, MapPin, Clock, X, User, Package, ShoppingCart } from 'lucide-react-native';
import { theme } from '../../theme';
import { apiService } from '../../services/api';

const ServiceProviderOrders = ({ user }) => {
    ...
    const handleSupply = async (orderId) => {
        try {
            await apiService.updateOrder(orderId, 'DELIVERED');
            fetchData();
        } catch (error) {
            Alert.alert('Error', 'Failed to mark order as supplied');
        }
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            >
                <Text style={styles.sectionLabel}>INCOMING SERVICE ORDERS</Text>
                    ...
                    >
                        ...
                        <View style={styles.actionRow}>
                            {order.status === 'PENDING' ? (
                                ...
                                <TouchableOpacity style={[styles.primaryAction, { backgroundColor: '#8B5CF6' }]} onPress={() => handleSupply(order.id)}>
                                    <Text style={styles.primaryActionText}>Mark as Supplied</Text>
                                </TouchableOpacity>
                            ) : (
                            ...
                            <ScrollView showsVerticalScrollIndicator={false}>
                                ...
                                <View style={styles.actionRow}>
                                    {selectedOrder.status === 'PENDING' ? (
                                        <TouchableOpacity style={styles.primaryAction} onPress={() => { handleAccept(selectedOrder.id); setSelectedOrder(null); }}>
                                            <Text style={styles.primaryActionText}>Accept Order</Text>
                                        </TouchableOpacity>
                                    ) : selectedOrder.status === 'PROCESSING' ? (
                                        <TouchableOpacity style={[styles.primaryAction, { backgroundColor: '#8B5CF6' }]} onPress={() => { handleSupply(selectedOrder.id); setSelectedOrder(null); }}>
                                            <Text style={styles.primaryActionText}>Mark as Supplied</Text>
                                        </TouchableOpacity>
                                    ) : null}
                                </View>
